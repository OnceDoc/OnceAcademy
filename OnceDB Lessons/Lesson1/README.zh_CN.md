...
我们的企业网盘 [OnceDoc](http://cn.oncedoc.com) 采用内存数据库Redis。Redis 是一个使用 C 语言编写的键值对存储数据库，体积小巧，性能优异，实施简单。很多大并发网站如Twitter、GitHub  Weibo、Snapchat、Flickr、阿里等都将其用作SESSION存储及缓存的管理。出于性能的考虑 Redis 自带的命令一般不支持按值查找。但是企业软件又需要数据库有搜索、复杂条件查询以及聚合分析的能力。为了实现这些功能我们修改了Redis的源码，并创建了一个新的开源分支 [OnceDB](https://github.com/OnceDoc/OnceDB)。

## 版本
  
我们基于Redis3.x版本进行了修改，尽管最新的4.x添加了对外部模块的支持，但并不适合我们的应用场景，外部模块会增加Reids崩溃的风险，并且会增加修改的难度。Redis从3.x版本开始支持集群Cluster，足以满足我们的需要。
...
string是redis最基本的类型，而且string类型是二进制安全的。即string可以包含任何数据。比如jpg图片或者序列化的JSON对象。

    var client = require("oncedb-client").createClient();

    //在 text* 类型的key中查找值为'Kris'的数据
...
比如查询 nVisit > 100， content含有 libs.min.js 的记录。"~|" 表示，截取过长的文本：
